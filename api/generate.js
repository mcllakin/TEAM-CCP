...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const packageImage = images.package || null;
        const compositionGuide = images.composition_guide || null;
        const mode = body.mode || 'ref-composition';

        // Modes that don't require a background reference (single-image refinement modes)
        const singleImageModes = new Set(['bg-color', 'perfume-angle', 'perfume-glass']);

        if (productSources.length < 1) {
            return res.status(400).json({ success: false, error: '제품 이미지가 필요합니다.' });
        }
        if (!backgroundReference && !singleImageModes.has(mode)) {
        ...
        console.log(`   package: ${packageImage ? 'yes' : 'no'}`);
        console.log(`   composition_guide: ${compositionGuide ? 'yes' : 'no'}`);
        console.log(`   count: ${safeCount}, aspect: ${aspect_ratio}`);
        console.log(`   product: ${identity.product_name || '(none)'}, item: ${identity.additional_item || '(none)'}\n`);

        // Build parts: order matters for the model.
        // Composition guide first (if any), then mood reference, then product sources, then package, then GWP.
        const parts = [];
        ...
        if (packageImage) {
            parts.push({ text: 'Image — Package Composition (the box and ribbon as they ship — preserve design, color, material, and brand mark exactly):' });
            parts.push(toInlinePart(packageImage));
        }
