...
async function callNanoBananaPro(requestData) {
    ...
    try {
        ...
        console.log('📦 받은 images 배열:', data.images);
        console.log('📏 배열 길이:', data.images?.length);
        console.log('========================================');
...
function displayResults(images) {
    ...
    if (!Array.isArray(images)) {
        console.error('❌ images가 배열이 아닙니다!', images);
        return;
    }
    
    console.log(`✅ ${images.length}개 이미지 표시 시작...`);
    
    // Add new results
    images.forEach((imageUrl, index) => {
        console.log(`  [${index + 1}] 이미지 추가 중: ${imageUrl?.substring(0, 50)}...`);
